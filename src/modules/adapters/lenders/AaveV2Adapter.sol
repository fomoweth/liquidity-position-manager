...
contract AaveV2Adapter is ILender, BaseLender {
	...
	function claimRewards(bytes calldata) public payable returns (uint256 rewards) {
		address incentives = INCENTIVES;

		bytes memory markets = abi.encode(getMarketsIn());

		rewards = getPendingRewards(incentives, markets);
	...
	function getPendingRewards(bytes calldata) external view returns (uint256) {
		return getPendingRewards(INCENTIVES, abi.encode(getMarketsIn()));
	}
	...
	function getAssetsIn() external view returns (Currency[] memory assets) {
		address lendingPool = LENDING_POOL;
		...
		if (userConfig != 0) {
			...
			unchecked {
				assets = new Currency[](length);

				while (i < length) {
					if (isSupplying(userConfig, i) || isBorrowing(userConfig, i)) {
						assets[count] = reserves[i];
						count = count + 1;
					}
	...
	function getReservesList() external view returns (Currency[] memory assets) {
