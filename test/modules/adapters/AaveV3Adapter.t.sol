...
// forge test -vvvv --match-path test/modules/adapters/AaveV3Adapter.t.sol
...
contract AaveV3AdapterTest is BaseTest {
	...
	function setUp() public virtual override {
		...
		wethMarket = getAaveMarket(WETH, true);
		daiMarket = getAaveMarket(DAI, true);

		rewardAssets = adapter.getRewardsList();
	}
	...
	function test_enableMarket() public {
		...
		adapter.enableMarket(wethMarket.underlying, false);

		assertTrue(!adapter.isSupplying(wethMarket.underlying));

		adapter.enableMarket(wethMarket.underlying, true);
